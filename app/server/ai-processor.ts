    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          // Re-throw — returning fake empty BoQ hides parse failures from RFI pipeline
          throw new Error(
            `BOQ JSON PARSE FAILURE: ${(error as Error).message}. ` +
            `An RFI must be raised — BoQ cannot be produced without valid structured data.`
          );
        }
      }
